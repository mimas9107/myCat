...
class VoiceWorker(QThread):
    ...
    def __init__(self, config_path=None, parent=None):
        ...
        self.vad = EnergyVAD(threshold=vad_cfg.get("threshold", 1000000.0))
    ...
    def run(self):
        ...
        while self._is_running:
            ...
            # 2. Wake Word Engine
            triggered, _score = self.wake_word.predict(audio_buffer)
            if triggered:
                self.status_changed_signal.emit("WAKE_WORD_TRIGGERED")

                # Record audio for command segment
                time.sleep(3.0)
                cmd_audio = self.audio_stream.get_buffered_audio()

                # 3. Speech Transcription
                self.status_changed_signal.emit("TRANSCRIBING")
                transcription = self.asr.transcribe(cmd_audio)

                # 4. Intent Parsing & Signal Dispatch
                if transcription:
